...
class module_information(public_base):
	...
	def syntax_highlighted_source(self):
		...
		self._syntax_highlighted_source = get_highlighted_terminal_from_lines(self.source)
	...
	def search(self, text):
		for index, line in enumerate(self.source, 1):
			if text in line:
...
class module_directory(public_base):
	...
	def from_package_name(cls, pkg_name):
		...
		for module in pkg.registry.values():
			module_registry[module.full_name] = module_information(module.full_name, module.path, module.stat, module.source.split('\n'), module.ast)
...
for r in MD.search('eval'):
	#print(r.module.full_name)
	r.present()
