...
UNDEFINED = register_symbol('internal.undefined')
FIRST_AVAILABLE = register_symbol('internal.first_available')
LAST_AVAILABLE = register_symbol('internal.last_available')
...
class config_ro_access:
	...
	def _conditional_process_settings(self, processors, conditions):
		for key, value in tuple(self._target.items()):	#local copy for mutation
			if value_condition := conditions.get(key):
				if isinstance(value, weak_and_lazy):
					self._target[key] = value = value.resolve()

				if value_condition(value):
					self._target[key] = processors[key](value)
				else:
...
def get_settings_from_instance(instance):
	result = dict()

	for base in reversed(instance.__class__.mro()):
		for field in base.__dict__.values():
			if isinstance(field, RTS.field) and field.field_type == RTS.SETTING:
