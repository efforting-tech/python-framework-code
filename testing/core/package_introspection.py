#TODO - move this stuff into library
...
cached_property = lambda: property #TODO

class ast_interface(R.Record):
	target: R.Field()
	node: R.Field()
...
class common_interface(R.Record):
	...
	def ast_lut(self):
		...
		def register(key, value):
			...
			lut[key] = ast_interface(self, value)
		...
		for node in self.ast.body:
			if isinstance(node, ast.Assign):
				...
				warnings.warn(f'Unhandled assignment in {self.full_name}: {node}')

			elif isinstance(node, (ast.Import, ast.ImportFrom)):
				for n in node.names:
					register(n.asname or n.name, node)

			elif isinstance(node, (ast.ClassDef, ast.FunctionDef)):
				register(node.name, node)

			else:
				warnings.warn(f'Unhandled AST node in {self.full_name}: {node}')
...
class package_info(common_interface):
	...
	path: R.Field()
	spec: R.Field()
	submodule_search_locations: R.Field()
	parent: R.Field() = None

	sub_modules: R.Field(factory=dict)
	registry: R.Field(factory=dict)


	#TODO
	# @property
	# def data_path(self):
	# 	return data_path(self.full_name)
	...
	#@R.initializer	#TODO - we will call it manually for now
	def initialize(self):
...
class module_info(common_interface):
	...
	@property
	def data_path(self):
		return self.parent.data_path / self.name
