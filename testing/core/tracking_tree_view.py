...
n = tuple(t.iter_nodes())
print(render_non_printables(n[1].body.sub_view.to_str() + '\n'))

# def dump_tree(i, t):
# 	for node in t.iter_nodes():
# 		print(i*'  ', node)
# 		dump_tree(i+1, node.body)
...
# Tree_View_Interface(line_view=Immutable_Line_View(…) index=0 length=2 empty=True)
# Tree_View_Interface(line_view=Immutable_Line_View(…) index=2 length=5 empty=False)
# Tree_View_Interface(line_view=Immutable_Line_View(…) index=7 length=1 empty=False)
# Tree_View_Interface(line_view=Immutable_Line_View(…) index=8 length=2 empty=True)
